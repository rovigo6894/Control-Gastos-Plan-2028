...
SUBCATEGORIAS = {
    "Alimentacion": ["Despensa", "Comida fuera", "Antojos", "Bebidas", "Cafe"],
    "Servicios": ["Luz", "Agua", "Gas", "Internet", "Telefono", "Netflix"],
    "Vivienda": ["Renta", "Mantenimiento", "Articulos hogar", "Limpiadores", "Muebles"],
    "Transporte": ["Gasolina", "Mantenimiento auto", "Pasajes", "Estacionamiento", "Taxi"],
    "Ahorro": ["Ahorro mensual", "Fondo emergencia", "Inversiones", "Meta especifica"]
}
...
st.caption("Subcategorias | Presupuestos editables | Alertas inteligentes")
...
if not df.empty:
    ...
    col2.metric("Proyeccion", f"${proyeccion:,.0f}")
    col3.metric("Presupuesto restante", f"${restante:,.0f}")

st.divider()

with st.expander("Editar presupuestos mensuales", expanded=False):
    st.subheader("Ajusta tus presupuestos por categoria")
    nuevos_presupuestos = {}
    cols = st.columns(2)
...
rubro = st.selectbox("Categoria", list(presupuestos.keys()))

subcategoria_opciones = SUBCATEGORIAS[rubro]
subcategoria = st.selectbox("Subcategoria", subcategoria_opciones)
...
with col1:
    if st.button("Guardar", use_container_width=True):
        if monto > 0:
            ...
            st.warning("Ingresa un monto valido")

with col2:
    if st.button("Eliminar ultimo", use_container_width=True):
        eliminar_ultimo()
        st.warning("Ultimo gasto eliminado")
        st.rerun()
...
if not df.empty:
    ...
    fig = px.pie(resumen, names="rubro", values="monto", title="Distribucion de gastos por categoria")
    st.plotly_chart(fig, use_container_width=True)
    
    st.subheader("Detalle por subcategoria")
    ...
    st.info("No hay gastos registrados aun. Agrega tu primer gasto arriba.")

st.divider()
st.caption("Control Financiero PRO | Subcategorias | Presupuestos editables | Alertas en tiempo real")
